...
public abstract class AbstractDeployableContainer implements DeployableContainer<JBossAsContainerConfiguration> {
    ...
    protected ProtocolMetaData getProtocolMetaData(Archive<?> archive, String uniqueDeploymentName)
    {
        ProtocolMetaData protocol = new ProtocolMetaData()
            .addContext(
                    new HTTPContext(containerConfig.getBindAddress().getHostAddress(), containerConfig.getHttpPort())
                        .add(new Servlet("ArquillianServletRunner", getContextRootName(archive))));
